...
const AddOrUpdateUser = ({ type = 'ADD' }) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (type === 'ADD') {
        // await postAdd();
        toast.success('ADD User To Existed Own List successfully');
      } else {
        // await update();
        toast.success('UPDATE User To Existed Own List successfully');
      }
    } catch (error) {
      toast.error(error);
    } finally {
  ...
  return (
    <Form form={form} onFinish={onFinish} onFinishFailed={onFinishFailed} layout="vertical">
      <h1 className="text-lg font-semibold mt-[28px] mb-4 uppercase">
        {type === 'MOVE' ? 'Update to user' : 'Add to user'}
      </h1>

      <Form.Item label="User Name" name="UserName" rules={[{ required: true, message: 'Please input user name!' }]}>
        <Input />
      </Form.Item>
      ...
      <Form.Item label="User Description" name="UserDescription">
        <TextArea rows={4} />
      </Form.Item>
